...
class ChatManager:
    ...
    '''
    def old__message_receiver(self, connection: socket.socket):
        """
        
        Receives the data and pushes it to the queue
            ...
            try:
                ...
                if terminator_index != -1:
                    if not self.__incoming_queue.full():
                        self.__incoming_queue.put(received[:terminator_index])
                    else:
                        logging.error("Queue is full")
                
                received.split(bytes([Protocol.Flags.SEPARATOR]))[0]
                ...
                while True:
                    ...
                    terminator = total_message[-1:]

                    if terminator == bytes([Protocol.Flags.TERMINATOR]):
                        break
                        
                    if total_message[-1] == "EOD":
                        break
